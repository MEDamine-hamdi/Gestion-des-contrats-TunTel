...
function ContratTT() {
    ...
    const saveContract = async (contract) => {
        try {
            ...
            if (res.ok) {
                ...
                alert('Échec de la sauvegarde du contrat');
            }
        } catch (error) {
    ...
    return (
        <div style={containerStyle}>
            <h2 style={{ textAlign: "center", marginBottom: "20px" }}>Liste des contrats TT</h2>
                ...
                {espacett_tmp.map((contract, index) => (
                    <li key={index} style={listItemStyle}>
                        ...
                        <div>
                            ...
                            <button style={{ ...buttonStyle, backgroundColor: "#f44336" }} onClick={() => deleteContract(contract._id)}>Supprimer</button>
                        </div>
                    </li>
                ))}
            </ul>
